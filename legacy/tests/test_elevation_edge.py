...
class TestInstallerElevationEdge(unittest.TestCase):
    def setUp(self):
        from legacy import webjam_installer  # deferred — module-level import fails on non-Windows
        self.webjam_installer = webjam_installer

    @patch("legacy.webjam_installer.is_admin", return_value=True)
    @patch("legacy.webjam_installer.ctypes.windll.shell32.ShellExecuteW")
    ...
    @patch("legacy.webjam_installer.is_admin", return_value=False)
    @patch("legacy.webjam_installer.ctypes.windll.shell32.ShellExecuteW", return_value=31)
    @patch("legacy.webjam_installer.input", return_value="")
    ...
    @patch("legacy.webjam_installer.is_admin", return_value=False)
    @patch("legacy.webjam_installer.ctypes.windll.shell32.ShellExecuteW", return_value=33)
...
class TestLegacyLauncherElevationEdge(unittest.TestCase):
    def setUp(self):
        from legacy import webjam_launch_session, webjam_win_oneclick
        self.webjam_launch_session = webjam_launch_session
        self.webjam_win_oneclick = webjam_win_oneclick

    @patch("legacy.webjam_launch_session.is_admin", return_value=False)
    @patch("legacy.webjam_launch_session.ctypes.windll.shell32.ShellExecuteW", return_value=31)
    def test_launch_session_denied_exits_error(self, _shell_mock, _is_admin_mock):
        with self.assertRaises(SystemExit) as ctx:
            self.webjam_launch_session.elevate_if_needed()
        self.assertEqual(ctx.exception.code, 1)

    @patch("legacy.webjam_win_oneclick.is_admin", return_value=False)
    @patch("legacy.webjam_win_oneclick.ctypes.windll.shell32.ShellExecuteW", return_value=31)
