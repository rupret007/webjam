# WebJam Standard Test Procedure (2024, archived)

> **Archived:** This is the original October 2024 test procedure, written for
> the legacy `test_webjam.py` multi-run harness (a standalone Tkinter-era
> unittest runner that looped until it saw 3 consecutive 100%-pass runs).
> That harness and the modules it references (`jamulus_controller.py`,
> `webex_integration.py`) no longer exist in the current Qt Conductor app.
> It is kept here for historical reference only.
>
> For the current test workflow, see the **Current Canonical Workflow (2026)**
> section in `TEST_PROCEDURE.md` at the repo root.
