...
class TestMetricsServiceExportEdge(unittest.TestCase):
    ...
    def test_export_snapshot_creates_missing_directory(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(data["jamulus"]["state"], "Connected")
            self.assertNotIn("webex", data)
    ...
    def test_export_diagnostics_bundle_creates_zip_with_expected_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with zipfile.ZipFile(out_zip, "r") as zf:
                ...
                self.assertNotIn("room_context.json", names)
                self.assertNotIn("environment.json", names)
                self.assertNotIn("support_snapshot.json", names)
                self.assertNotIn("files/webjam.db", names)
                snapshot = json.loads(zf.read("support.json").decode("utf-8"))
                self.assertEqual(snapshot["jamulus"]["state"], "Connected")
                encoded = json.dumps(snapshot)
                self.assertNotIn("localhost:22124", encoded)
                self.assertNotIn("webex.example.com", encoded)
                self.assertNotIn("C:/Jamulus.exe", encoded)
