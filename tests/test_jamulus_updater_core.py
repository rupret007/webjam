...
def test_official_registry_authorizes_v0272_but_no_future_patch():
    registry = official_jamulus_compatibility_registry()
    ...
    assert not any(entry.supports_webjam("0.27.3") for entry in registry.entries)
    ...
    for target in ComponentTarget:
        for role in (JamulusRole.CLIENT, JamulusRole.SERVER):
            ...
            assert registry.compatible(
                role=role,
                target=target,
                webjam_version="0.27.3",
            ) == ()


def test_v0272_payload_reuses_approved_records_and_verifies(tmp_path: Path):
    ...
    private_path, keyring = _test_signer(tmp_path / "v0272-test-only")
    envelope = sign_component_catalog(
        payload,
        private_key_path=private_path,
        key_id="test-key",
    ...
    verified = verifier.verify(envelope, webjam_version="0.27.2")

    assert verified.sequence == 7
    assert verified.webjam_version == "0.27.2"
