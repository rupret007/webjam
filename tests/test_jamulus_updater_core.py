...
def test_official_registry_authorizes_exact_v0240_but_no_future_patch():
    registry = official_jamulus_compatibility_registry()

    assert all(entry.supports_webjam("0.24.0") for entry in registry.entries)
    assert not any(entry.supports_webjam("0.24.1") for entry in registry.entries)
    ...
    for target in ComponentTarget:
        for role in (JamulusRole.CLIENT, JamulusRole.SERVER):
            candidates = registry.compatible(
                role=role,
                target=target,
                webjam_version="0.24.0",
            )
            assert [item.version for item in candidates] == ["3.12.3", "3.12.2"]
            assert registry.compatible(
                role=role,
                target=target,
                webjam_version="0.24.1",
            ) == ()
