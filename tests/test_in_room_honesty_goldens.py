"""Exact in-room honesty goldens after #27.

Door law is unchanged and held by the start-UX suite. These goldens hold the
one leftover lie this pass closes: a song clock without a stated bar or
section is not form, even when a Shared Track is playing and the notes
already list parts.
"""
...
def test_written_outline_without_a_position_is_no_clock_golden():
...
def test_real_clock_outline_without_tempo_is_no_clock_golden():
    ...
    assert song_form_facts(snapshot) is None
    view = render_room_clock(song_form_facts(snapshot))
    assert view.headline == NO_CLOCK_HEADLINE
