"""Unsigned v0.28.2 candidate; GitHub Latest remains published v0.28.1."""
...
def test_v0282_candidate_while_v0281_remains_github_latest() -> None:
    match = re.search(
    ...
    assert match.group(1) == "0.28.2"
    assert application_version() == "0.28.2"
    ...
    assert "reports unsigned v0.28.2" in normalized
    ...
    assert "release ID `377546932`" in normalized
    assert "BlackHole" in normalized
    assert "sealed at exact WebJam v0.22.5" in normalized
...
def test_runtime_sbom_names_the_exact_desktop_version() -> None:
    ...
    assert component == {
        "bom-ref": "pkg:generic/webjam@0.28.2",
        "name": "WebJam",
        "purl": "pkg:generic/webjam@0.28.2",
        "type": "application",
        "version": "0.28.2",
    }
...
def test_component_sbom_names_the_exact_desktop_version() -> None:
    ...
    assert properties["webjam:current-source-authorization"] == (
        "baked-jamulus-records-approved-through-webjam-0.28.2"
    )
...
def test_payload_reuses_the_approved_v0280_records() -> None:
    ...
    assert payload["webjam_version"] == "0.28.2"
    ...
    assert all(
        component["webjam_range"]["maximum"] == "0.28.2" for component in components
    )
    assert all(
...
def test_current_guides_separate_v0281_latest_from_historical() -> None:
    expected = {
        "ARCHITECTURE.md": "# WebJam architecture — v0.28.2 source",
        "CHANGELOG.md": (
        ...
        "CREATIVE_MODES_MVP_SPEC.md": (
            "# Creator profiles — v0.28.2 implemented contract"
        ),
        "DEVELOPMENT.md": "# Developing WebJam v0.28.2",
        "FIRST_JAM.md": "release `393030220`",
        "HELP_ROUTING_MAP.md": "# WebJam help routing — v0.28.2 source",
        "QUICK_HELP_MAP.md": "# WebJam quick help — v0.28.2 source",
        "README.md": "release ID `393030220`",
        "README_SIMPLE.md": "393030220",
        "RECORDING_AND_STUDIO.md": "# Recording and Studio — v0.28.2 source",
        "SECURITY.md": "393030220",
        "TEST_PROCEDURE.md": "# WebJam v0.28.2 source test procedure",
        "USER_GUIDE.md": "# WebJam creator guide — v0.28.2 source",
        "UX_ACCEPTANCE_CHECKLIST.md": "# WebJam v0.28.2 source UX acceptance checklist",
        "V025_CREATOR_MULTITRACK_PHYSICAL_TEST_CHECKLIST.md": (
            "v0.25.0 was GitHub **Latest**"
        ),
        "WEBEX_AUDIO_MODES.md": (
            "# Meeting-platform companion guidance — v0.28.2 source"
        ),
        "docs/DESKTOP_RELEASE_RUNBOOK.md": "v0.28.1 published testing boundary",
        "docs/JAMULUS_COMPONENT_RELEASE_RUNBOOK.md": (
            "v0.28.2 candidate fallback-only desktop state"
        ),
        ...
        "docs/REFERENCE_STUDIO_MUSICIAN_GUIDE.md": "v0.28.2 source guide",
    }
...
def test_v0271_release_history_stays_immutable_without_invented_catalog() -> None:
    assert not (
        ROOT / ".github" / "workflows" / "publish-v027-testing-release.yml"
    ).exists()
    assert not (
        ROOT / ".github" / "workflows" / "publish-v0271-testing-release.yml"
    ).exists()
    section = CHANGELOG.split(
        "## [0.27.1] — Unsigned/ad-hoc private test release (2026-08-27)", 1
    )[1].split("\n## [0.27.0]", 1)[0]
    assert "377614785" in section
    assert "ba81f8ef65db1013f13773b1536c812af174d81f" in section
    assert "1fc25f87c3386b1cd94303ecb407cdaff6509d1f" in section
    assert "WebJam-v0.27.1-SHA256SUMS.txt" in section
    assert "33045632613" in section
    assert "overall run is red and must not be called publish-green" in section
    assert "Do not retag or mutate v0.27.1" in section
    assert "0.22.5" in section
    assert "BlackHole" in section
    assert "PINNED_" not in section
    assert "377546932" not in section
...
def test_changelog_marks_v0280_and_v0272_as_deleted_and_keeps_prior_history() -> None:
    ...
    assert "Tag workflow `33327104322`" in changelog
    assert "passed tests, integrations, and all four desktop builds" in normalized
    assert "publisher failed closed because it requires an annotated tag" in normalized
    ...
    assert "overall run is red and must not be called publish-green" in changelog
    assert "No v0.27.1 release ID, checksum, or pinned artifact hash exists" not in changelog
    assert "## [0.27.0] — Unsigned private test release (2026-08-27)" in changelog
    assert "377546932" in changelog
    assert "No v0.27.0 release ID, checksum, or pinned artifact hash exists" not in (
        changelog
    )
    assert (
...
def test_source_eligible_copy_is_explicit_about_platform_trust() -> None:
    ...
    runbook = (ROOT / "docs" / "DESKTOP_RELEASE_RUNBOOK.md").read_text(encoding="utf-8")
    assert "unsigned" in windows_readme.casefold()
    assert "SmartScreen" in windows_readme
    assert "ad-hoc" in macos_readme
    assert "signed" in macos_readme
    assert "NOT notarized" in macos_readme
    assert "Gatekeeper" in macos_readme
