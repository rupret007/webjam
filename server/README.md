...
One-click in-app hosting is macOS-only in the private pilot. The v0.15 package
candidate contains prepared nested client and server apps. The launch path
...
is **not** the v0.15 test-night validation path; that path uses the prepared
...
advanced source/developer topology; the v0.15 Join screen intentionally
...
- The `.rpp` file is a Reaper project; WebJam's own **Track Export** is a
  portable WAV package for an editor of the musician's choice. It refuses an
  explicitly silent selected stem or an unaligned guest/local original until
  the musician deselects it, keeps an aligned server stem, or aligns and
  verifies it. This is a source safety rule, not a completed external-editor
  import validation.
