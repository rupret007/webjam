...
## macOS host + musician workstation (v0.11.0 pilot)

**WebJam hosts the server automatically.** In the packaged macOS app, choose
**Host a Jam**; if Band Check appears, complete it and choose **Start Session**.
...
One-click in-app hosting is macOS-only in the v0.11.0 private pilot.
...
The remote musician does not receive the recorder secret. Tonight's v0.11.0
...
is **not** the v0.11.0 test-night validation path; that path uses the prepared
...
advanced source/developer topology; the v0.11.0 Join screen intentionally
