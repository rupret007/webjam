...
## macOS host + musician workstation (v0.14.0 private test-night candidate)

The v0.14.0 Apple-Silicon ZIP is the active private test-night package:
...
It is ad-hoc signed and not notarized. The source gate recorded 1,719 passed,
18 skipped, one known warning, and 6 subtests; native transport `go test ./...`
and `go vet ./...` passed. A fresh extraction passed strict/deep signatures,
nested-app inspection, exact native-fabric build-ID verification, and two
isolated six-second offscreen launch/TERM cycles. Physical CoreAudio, two-Mac,
recording/recovery, and Logic results remain **NOT RUN**. The v0.13.0 and older
ZIPs are rollback evidence only.
...
One-click in-app hosting is macOS-only in the private pilot. The active v0.14.0
...
is **not** the v0.14.0 test-night validation path; that path uses the prepared
...
advanced source/developer topology; the v0.14.0 Join screen intentionally
