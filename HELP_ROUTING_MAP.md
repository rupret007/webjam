...
| “Which creator profile should I choose?” | Use **Music** or **Podcast & Voice** for GA workflows. **Review & Rehearsal** is Preview: live WebJam-audio Host/Join, Record Session, local notes, and read-only take playback work; standalone projects, take editing/comp/mix mutation, track export, shared notes, visual sync, and media timecode do not. **Studio Visit** is Preview and is for artists working in any medium: live WebJam-audio Host/Join, local notes, and one optional host-clocked reference video work; standalone projects, session recording, every take capability, a shared canvas, and frame-accurate or timecoded review do not. No profile directly or automatically taps a meeting app, browser, or system output. |
| “Why will Studio Visit not play the video I opened?” | WebJam only follows a file it can prove is the host's exact file. A different file, or a copy that later moved, changed, or became unreadable, is refused rather than shown. Open the host's exact file, or choose **Hide Video** and stay in the room. |
...
| “Where is my Studio Visit recording?” | There is none. Studio Visit does not record a session, so it produces no take rather than a take whose sources cannot be proven. |
