# WebJam help routing — v0.28.3 source
...
| “Which Art start do I want?” | **Make together** to talk, make, or draw together in one room. Everyone may work locally, and the host can open one shared canvas from inside the room. **Paint along** starts with one host-clocked video. |
...
| “Can I use AI to make or fix an image?” | Yes, from inside an Art session: **Suggestion** on the notes offers **Make** and **Edit**. Krita's AI Image Generation plugin does the work on your computer; WebJam finds it, opens it, and takes no prompt. Install Krita 5.2.0+ and Krita AI Diffusion first; WebJam says which one is missing. |
...
| “Why is AI not a start card?” | Because nobody decides what they are making by choosing an image generator. Both starts can reach **Suggestion** on the notes once you are in the room. |
...
| “Why is Join still trying to connect?” | A remote join is bounded and must leave its progress states for **Needs attention**. Use its one action: **Try Again** only when WebJam proved the invitation was not used; otherwise ask the host for a new invitation and choose **Paste New Invite**. Do not post the old invitation in an issue. |
| “Why can’t I paste the same invitation again?” | Remote invitations are short-lived, one-use private capabilities. After timeout, expiry, or uncertain enrollment, replay would be unsafe. Ask the host for a new invitation. |
| “Can I chat while connection setup is failing?” | Not yet. Band chat is Jamulus chat and appears only after authenticated Jamulus connection. A bounded pre-connection text channel requires a separate authenticated transport slice; WebJam does not fake one. |
...
| “Can we follow a demonstration together?” | In an Art room, open **Conversation**, then **Join / Open Meeting**. Share a demonstration in Webex or the selected meeting app. Everyone can use paint, clay, paper, a printer, or their own software. **Paint along** is the separate silent local-file option. |
| “My notes will not save.” | Keep WebJam open and choose **Notes → Save Notes**. The recovery window keeps drafts by workspace, including a previous workspace. Shorten long notes or **Export Copy…** to a different file. An unreadable saved original is preserved; new notes can be exported without replacing it. |
| “Why does setup say Close Setup?” | The earlier setup has not finished closing. Choose **Close Setup** to retry cleanup. WebJam will not start another connection while that cleanup is unconfirmed. |
| “Recording cannot use my selected inputs.” | Choose **Recording Setup** on the readiness sheet. No recording starts; fix the inputs and choose **Record Session** again for a fresh check. |
| “Where is my local Music project?” | Before starting a session, choose **File → New Music Project…**. Podcast and Review are also in the launch File menu. A Join recovery window only accepts a replacement invitation. |
| “Where is Studio?” | Use the direct **Studio** action or Cmd/Ctrl+3. Studio is intentionally absent from More. |
| “Why will my Shared Track not play?” | Source loading and playback permission are separate. After a local file loads, a missing isolated route says **Set up the audio device** and opens Shared Track. Install official BlackHole 16ch or 64ch at 48 kHz, then **Recheck Route**. That proof is this Mac's device, not a signed catalog or `webjam_version` pin. Loading still validates the file and decodes its first bounded block. |
