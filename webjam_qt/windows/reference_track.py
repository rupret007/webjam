...
class ReferenceTrackDialog(QDialog):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self._trim.editingFinished.connect(
            lambda: self.trim_requested.emit(float(self._trim.value()))
        )
        ...
        safety.setObjectName("DialogHint")
        safety.setWordWrap(True)
        safety.setTextFormat(Qt.TextFormat.PlainText)
    ...
    def _emit_count_in(self) -> None:
        if self._syncing:
            return
        self.count_in_requested.emit(
            int(self._count_in.value()),
            float(self._count_bpm.value()),
        )
    ...
    def set_snapshot(self, snapshot: object) -> None:
        ...
        self._route.setText(route_detail or capability_detail)
        ...
        try:
            ...
            self._loop.setChecked(loop_end_raw is not None)
            if not self._spin_box_has_focus(self._loop_start):
                self._loop_start.setValue(min(duration, loop_start))
